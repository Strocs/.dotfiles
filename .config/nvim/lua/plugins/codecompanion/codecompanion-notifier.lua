...
local function start(ev)
  ...
  if timer ~= nil and type(timer.start) == "function" then
    if type(refresh) == "function" then
      timer:start(0, 150, vim.schedule_wrap(refresh))
    else
      vim.notify("refresh is not a function", vim.log.levels.ERROR)
    end
  else
    vim.notify("timer is nil or invalid", vim.log.levels.ERROR)
  end
  if type(refresh) == "function" then
    refresh()
  end
